...
<div class="container" style="border: 2px solid black; background: #9575CD">
<?php
...
function dfs(Node $node, Node $destination, $path = '', $visited = array())
{
    ...
    if($node->name==$destination->name)
    {
        ...
        {
            echo '<span id=special style="margin: 5px; padding: 5px; border-radius: 5px; text-transform: capitalize">'.'<b>'.$city.'</b>'.'</span>';

            echo '<span class="fa fa-arrow-right" style="color: white;"></span>'; 
        }
        echo '<span id=special style="margin: 5px; padding: 5px; border-radius: 5px; text-transform: capitalize">'.'<b>'.$node->name.'</b>'.'</span>';
        // echo 'path : ' . $path . ' ' . $node->name ."<b>Total Cost: </b>";
        echo  '<span style="float: right;">'."<b>Total Cost:".$total_cost.' </b></span>'."<br><br>";
        return;
    }
